...
/*		Query query = new Query("stackoverflow");
		...
		String queryString = "Carlsen";
		Query query = new Query(queryString);
		query.count(10); // sets the number of tweets to return per page, up to a max of 100
	    QueryResult result;
		try {
			result = twitter.search(query);
			Integer countTw = 1;
...
//		        System.out.println(countTw++ + " > @" + status.getUser().getScreenName() + " (" + status.getCreatedAt().toString() + ") : " + status.getText() + "\n");
			...
			if(result.hasNext())
	        {
	        	System.out.println("girdiqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq");
	        	for (Status status : result.getTweets()) {
//			        System.out.println(countTw++ + " > @" + status.getUser().getScreenName() + " (" + status.getCreatedAt().toString() + ") : " + status.getText() + "\n");
			        System.out.println(countTw++ +" "+ status.getText() + "\n ");			      		        
			    }
